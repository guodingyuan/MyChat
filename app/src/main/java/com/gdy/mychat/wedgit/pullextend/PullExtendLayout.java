...
public class PullExtendLayout extends LinearLayout implements IPullToExtend {
    ...
    protected void onFinishInflate() {
        ...
        if(mHeaderLayout!=null){
            mHeaderLayout.setResetLayout(new ExtendLayout.ResetLayout() {
                @Override
                public void onRest() {
                    smoothScrollTo(0);//滑动到原来的位置，初始状态
                }
            });
        }
    ...
    private void refreshLoadingViewsSize() {
       ...
       /* if(headerListHeight>headerHeight){
            headerListHeight=headerHeight;//不能超过它
        }*/

       //重点：距离变化后必须进行重新的滚动
       if(getScrollYValue()!=0 && -headerListHeight!=getScrollYValue()){
           smoothScrollTo(-headerListHeight);
       }


Log.e("gdy","目前滑动距离："+getScrollYValue());
        int pLeft = getPaddingLeft();
        int pTop = -headerListHeight;
        int pRight = getPaddingRight();
        int pBottom = -footerListHeight;
    ...
    protected void refreshRefreshableViewSize(int width, int height) {
        ...
        /*if(null != mHeaderLayout){
            ViewGroup.LayoutParams headLP=mHeaderLayout.getLayoutParams();
            headLP.height= height;//mHeaderLayout.getListSize()>height ? height:mHeaderLayout.getListSize();//高度不能超过
            mHeaderLayout.setLayoutParams(headLP);
        }*/
    }
    ...
    public final boolean onInterceptTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if(mHeaderLayout!=null && mHeaderLayout.childScrollView!=null && mHeaderLayout.getState()== IExtendLayout.State.arrivedListHeight){
                    ...
                    if(mHeaderLayout.childScrollView instanceof RecyclerView){
                        ...
                        //Log.e("gdy","可上滑："+mRecyclerView.canScrollVertically(1)+"，可下滑："+mRecyclerView.canScrollVertically(-1));
                        if((mRecyclerView.canScrollVertically(1) && deltaY<0) || (mRecyclerView.canScrollVertically(-1) && deltaY>0)){
                            //注意进行方向的判定
                            return false;
                        }
                    }else if(mHeaderLayout.childScrollView instanceof NestedScrollView /*|| mHeaderLayout.childScrollView instanceof ScrollView*/){
    ...
    protected void resetHeaderLayout() {
        ...
        if(mLastMotionY-mFirstDownY<0){//上滑
            smoothScrollTo(0);
        }else {//下拉
